...
if build "pkg-config" "0.29.2"; then
  ...
  execute ./configure --silent --prefix="${WORKSPACE}" --with-pc-path="${WORKSPACE}"/lib/pkgconfig:/usr/lib/aarch64-linux-gnu/pkgconfig:/usr/lib/pkgconfig:/usr/local/lib/pkgconfig:/usr/share/c4_amllibs/usr/lib/pkgconfig:/usr/share/pkgconfig --with-internal-glib
  execute make -j $MJOBS
...
if build "cmake" "3.22.0"; then
  ...
  execute ./configure --prefix="${WORKSPACE}" --parallel="${MJOBS}" -- -DCMAKE_USE_OPENSSL=ON
  execute make -j $MJOBS
  execute make install
  build_done "cmake" "3.22.0"
fi

if build "libdrm" "2.4.110"; then
  download "http://dri.freedesktop.org/libdrm/libdrm-2.4.110.tar.xz"
  make_dir build
  execute meson build --prefix="${WORKSPACE}" -Dlibkms=false -Dnouveau=true -Domap=false -Dexynos=false -Dcairo-tests=false -Dman-pages=false -Dvalgrind=false -Dfreedreno-kgsl=false -Dinstall-test-programs=true -Dudev=false --libdir="${WORKSPACE}"/lib
  execute ninja -C build/ install
  build_done "libdrm" "2.4.110"
fi

if build "mesa" "21.0.3"; then
  download "https://mesa.freedesktop.org/archive/mesa-21.0.3.tar.xz"
  make_dir build
  execute meson build --prefix="${WORKSPACE}" -Dgallium-extra-hud=false -Dgallium-xvmc=disabled -Dgallium-omx=disabled -Dgallium-nine=false -Dgallium-opencl=disabled -Dshader-cache=enabled -Dshared-glapi=enabled -Dopengl=true -Dgbm=enabled -Degl=enabled -Dvalgrind=disabled -Dlibunwind=disabled -Dlmsensors=disabled -Dbuild-tests=false -Dselinux=false -Dosmesa=false -Dplatforms="" -Ddri3=disabled -Dglx=disabled -Dglvnd=false -Dllvm=disabled -Dgallium-vdpau=disabled -Dgallium-va=disabled -Dgallium-xa=disabled -Dgles1=disabled -Dgles2=enabled
  execute ninja -C build/ install
  build_done "mesa" "21.0.3"
fi
...
download "https://github.com/FFmpeg/FFmpeg/archive/refs/tags/n4.4.1.tar.gz" "FFmpeg-release-$FFMPEG_VERSION.tar.gz"
...
#install libmali (gives you much slimmer install of satisfying need for libegl-dev)
#install shairplay
#install Dav1d

#there is an issue with the original EGL and GLES files located in /usr/local/includes/EGL/ and /usr/local/includes/GLES/.
# For some reason, there is a discrepency about a number or float conversion declaration, and this causes kodi to not want to build the GBM windowing system.
# however, compilation can still make progress if you install libegl-dev and libegl1-mesa-dev packages, and then rename all of the files in
# /usr/local/includes/EGL/ and /usr/local/includes/GLES/ to have the prefix "old." in front of the filename.  and then reload the library with ldconfig.
#  2022-03-15 :: it is unknown to me if these new files will allow a successful kodi windowing system to be built